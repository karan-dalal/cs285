...
class MPCPolicy(BasePolicy):
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        cumulative = np.zeros(self.N)
        for model in self.dyn_models:
            cumulative += self.calculate_sum_of_rewards(obs, candidate_action_sequences, model)

        return cumulative / len(self.dyn_models)
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        """
        ob_no = np.repeat(obs.squeeze()[None], candidate_action_sequences.shape[0], axis=0)
