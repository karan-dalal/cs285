...
class RL_Trainer(object):
    ...
    def __init__(self, params):
        ...
        if self.params['video_log_freq'] > 0:
            self.episode_trigger = lambda episode: episode % self.params[
                'video_log_freq'] == 0
        else:
    ...
    def run_sac_training_loop(self, n_iter, collect_policy, eval_policy):
        ...
        for itr in range(n_iter):
            ...
            if self.logvideo or self.logmetrics:
                # perform logging
                print('\nBeginning logging procedure...')
                self.perform_sac_logging(itr, episode_stats, eval_policy,
                                         train_video_paths, all_logs)
                episode_stats = {'reward': [], 'ep_len': []}
                if self.params['save_params']:
                    self.agent.save('{}/agent_itr_{}.pt'.format(
    ...
    def perform_sac_logging(self, itr, stats, eval_policy, train_video_paths,
                            all_logs):
